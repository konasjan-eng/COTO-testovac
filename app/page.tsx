        ...
        <div className="tvl-right">
          <div className="survey-box">
            ...
            <div className="project-list window-c">
              ...
              <div className="project-columns"><span>Návrh nebo otázka</span><b>1–9</b></div>
              {projects.map((project, i) => (
                <button key={i} className="project-row" onClick={() => onInspect(i)} title="Kliknutím otevřete celý popis">
                  <span>{i + 1}</span><strong>{project.title || "Klikněte a zapište projekt nebo otázku"}</strong><span className="manager-priority">{project.value} Kč</span><span className="participant-score">{participantScores?.[i] ? participantScores[i] : ""}</span>
                </button>
              ))}
            </div>
          </div>
...
export default function Home() {
  ...
  if (entryStage === "icon") return <main className="entry-screen"><button className="coto-entry-icon video-look" onClick={() => setEntryStage("logo")} aria-label="Otevřít COTO"><img src="/COTO-testovac/coto-icon-original.png" alt="Původní ikona COTO – otevřít aplikaci" /><span>KLIKNUTÍM OTEVŘÍT COTO</span></button></main>;
  if (entryStage === "logo") return <main className="entry-screen"><button className="coto-entry-logo coto-entry-logo-image" onClick={() => setEntryStage("roles")} aria-label="Pokračovat z loga COTO k volbě role"><img src="/COTO-testovac/coto-uvod.png" alt="COTO – Codáš a Tomáš, aplikace pro spravedlivou výměnu informací" /><span className="logo-continue">KLIKNUTÍM POKRAČUJTE</span></button></main>;
  ...
  if (entryStage === "variants") return <main className="entry-screen"><div className="entry-panel variant-panel"><p className="eyebrow">SPRÁVCE · VÝBĚR VARIANTY COTO</p><h1>Čtyři varianty</h1><button className="variant-choice active" onClick={() => setEntryStage("app")}><b>PN</b><span>Průzkum názorů</span><em>OTEVŘÍT PRACOVNÍ ŠABLONU</em></button><button className="variant-choice" disabled><b>VL</b><span>Vyber lepší</span><em>PŘIPRAVUJEME</em></button><button className="variant-choice" disabled><b>PP</b><span>Podpora projektu</span><em>PŘIPRAVUJEME</em></button><button className="variant-choice" disabled><b>VT</b><span>Volební tombola</span><em>PŘIPRAVUJEME</em></button><button className="muted" onClick={() => setEntryStage("ares")}>ZPĚT</button></div></main>;
  ...
  if (entryStage === "readonly") return <main className="readonly-screen"><div className="readonly-toolbar"><button onClick={() => setEntryStage("dashboard")}>ZPĚT NA PŘEHLED</button><button onClick={() => navigator.clipboard?.writeText(`${title}\n${projects.map((p, i) => `${i + 1}. ${p.title}: ${p.detail}`).join("\n")}`)}>KOPÍROVAT OBSAH</button></div><div className="tvl-paper original-a4"><TvlSection kind="voucher" {...sharedTvl} onInspect={setSelected}/><div className="cut">✂</div><TvlSection kind="investment" {...sharedTvl} onInspect={setSelected}/><div className="cut">✂</div><TvlSection kind="receipt" {...sharedTvl} onInspect={setSelected}/></div></main>;
    ...
    <main>
      ...
      <div className="workspace">
        ...
        <div className="paper-wrap">
          ...
          <div className="tvl-paper compact original-a4">
